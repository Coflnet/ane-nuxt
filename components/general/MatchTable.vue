<template>
  <UiDefaultContainer class="mb-6 p-6" v-if="props.matches && props.matches.length > 0">
    <table class="w-full">
      <TableHeader />
      <tbody>
        <tr v-for="(auction, index) in props.matches" :key="index"
          ...
          <td class="px-4 py-3 text-sm text-slate-900 dark:text-white" v-if="'filter' in auction">{{
          ...
          <td class="px-4 py-3 text-sm font-medium text-slate-900 dark:text-white" v-if="description(auction)">{{
            description(auction) }}</td>
          <td class="px-4 py-3 text-sm text-slate-500 dark:text-slate-400">{{ timeAgo(auction) }}</td>
          <td class="px-4 py-3 text-sm">
            <div class="flex items-center space-x-2 ml-2.5">
              <a :href="url(auction)" target="_blank" class="p-1 rounded-md hover:bg-slate-100 dark:hover:bg-slate-700"
                title="View auction">
