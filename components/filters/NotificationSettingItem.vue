<template>
  <DefaultInputBox class="flex items-center p-4" :accent="true">
    <div class="flex items-center flex-col h-full justify-center">
      <input id="email-notification" v-model="filter.notificationType" type="radio" :value="config.notificationType"
        @click="filter.notificationTarget = config.predefinedValue"
        class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-slate-300" />
    </div>
    <label for="email-notification" class="ml-3 flex">
