  ...
  <div class="max-w-md mx-auto mt-12">
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-lg overflow-hidden">
      <div class="p-8">
        <div class="text-center mb-8">
          ...
          <p class="mt-2 text-slate-500 dark:text-slate-400">{{ $t('registerHead') }}</p>
        </div>
        ...
        <div class="space-y-4">
          <button @click="loginWithGoogle"
            ...
            <img src="https://www.gstatic.com/firebasejs/ui/2.0.0/images/auth/google.svg" alt="Google logo"
              class="w-5 h-5" />
            <span class="text-slate-700 dark:text-white font-medium">{{ $t('signupGoog') }}</span>
          </button>
          ...
          <div class="text-center text-sm text-slate-500 dark:text-slate-400">
            <p>{{ $t('sellSoul') }} <a href="https://coflnet.com/terms-of-service"
                ...
                class="text-indigo-600 dark:text-indigo-400 hover:underline">{{ $t('pp') }}</a>.</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="mt-6 text-center">
      <p class="text-slate-500 dark:text-slate-400">
        {{ $t('already') }}<NuxtLink to="/login" class="text-indigo-600 dark:text-indigo-400 hover:underline">
          {{ $t('signinNormal') }}
        </NuxtLink>
      </p>
    </div>
